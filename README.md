...
- **成长里程碑**：
  ...
  - **自动关联照片**：新建或选中建议里程碑时，`MilestonePhotoMatcher` 会先按内容关键词过滤（比如「第一个生日」找自动标签含"生日 / 生日蛋糕 / 蛋糕 / 派对"的照片；「第一次走路」找"走路"），再在命中的照片里按拍摄日期最接近里程碑日期的一张预填进去；没有任何内容命中时退化到纯按日期最近。改日期会自动重新匹配，直到你主动选了别的照片为止
...
3. **内容感知的照片自动匹配（`MilestonePhotoMatcher`）**
   - `MilestoneCatalog.Entry` 里每条可以挂一组 `photoKeywords`，对应 Vision 场景分类翻译后的中文标签：
     - 「第一次微笑」→ `["微笑"]`
     - 「第一次吃辅食 / 第一次自己吃饭」→ `["吃饭", "食物", "餐椅", "餐食"]`
     - 「第一次独立走路 / 第一次跑」→ `["走路"]`
     - 「第 N 个生日」→ `["生日", "生日蛋糕", "蛋糕", "派对"]`
     - 「第一次骑三轮车」→ `["自行车"]`
   ...
   - `MilestoneEditView` 顶部的「已自动匹配」旁边会显示是"内容匹配「生日蛋糕」· 拍摄于里程碑当天"还是单纯"拍摄于里程碑日期前 3 天"
   - 手写标题（没在 catalog 里的）查不到关键词，自动退化到纯按日期匹配
