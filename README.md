...
此外，过滤器本身也默认偏严：忽略占画面 < 0.3% 的小背景脸，要求「最像女儿的脸」比任何排除脸都明显更像她（`contrastMargin = 2`，不是"只强一点点"）。这些默认值在 `FaceRecognitionService.matchResult` 里。
...
2. **更精准**：设置 → **排除人脸** → 加一张把你（妈妈/爸爸）自己的正脸照，再按下面 Q 里的步骤把已有照片复核一遍。对比式匹配会自动把「更像你而不是像女儿」的照片过滤掉，不需要动阈值。可以加多张（爸爸、外公、外婆……）。
...
默认的过滤已经：忽略占画面 < 0.3% 的小脸、并要求「最像女儿的那张脸」比任何排除脸都明显更像她（`contrastMargin = 2`，不是"只强一点点"）。如果还有漏网的：
